...
func runDescribe(cmd *cobra.Command, args []string, newClient ClientFactory) (err error) {
	...
	if cfg.Name != "" { // Describe by name if provided
		...
		// By-path describe targets the function's pinned namespace (Describe
		// resolves f.Deploy.Namespace), so pin the client to it, not active.
		kc, err := newK8sClientFromConfig(cfg.Cluster, cfg.ClusterToken, f.Deploy.Namespace, f.Local)
		if err != nil {
			return err
		}
