...
// not the intent that was asked for. A pipeline whose func-util predates
...
func TestDeploy_RemoteExposeRecordsObservation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cmd.SetOut(&out)
			cmd.SetErr(&out)
			cmd.SetArgs([]string{"--remote", "--git-url=https://example.com/user/repo",
...
// TestDeploy_ExposeRouteUnreachableClusterIsNotAPlatformClaim: the gate
// refusing --expose=route must not tell the user their cluster is not
// OpenShift when it never got an answer. That sentence is a claim about their
// cluster, and an unreachable API server is a claim about their connection.
//
// The refusal itself must stay: a Route on a cluster that may not serve Routes
// is exactly what this gate exists to prevent. Only the stated cause changes.
