...
func TestNew(t *testing.T) {
	...
	// New Client
	client := function.New(function.WithRegistry(TestRegistry))

	// New Function using Client
	if err := client.New(function.Function{Root: root}); err != nil {
		t.Fatal(err)
	}
}
...
func TestTemplateWrites(t *testing.T) {
	...
	client := function.New(function.WithRegistry(TestRegistry))
	if err := client.Create(function.Function{Root: root}); err != nil {
		t.Fatal(err)
	}

	// Assert file was written
	if _, err := os.Stat(filepath.Join(root, function.ConfigFile)); os.IsNotExist(err) {
		t.Fatalf("Initialize did not result in '%v' being written to '%v'", function.ConfigFile, root)
	}
}
...
func TestDefaultRuntime(t *testing.T) {
	...
	if f.Runtime != function.DefaultRuntime {
...
func TestExtensibleTemplates(t *testing.T) {
	...
	client := function.New(
		function.WithTemplates("testdata/templates"),
		function.WithRegistry(TestRegistry))

	// Create a Function specifying a template, 'json' that only exists in the extensible set
	if err := client.New(function.Function{Root: root, Trigger: "boson-experimental/json"}); err != nil {
		t.Fatal(err)
	}
...
func TestUnsupportedRuntime(t *testing.T) {
	...
	if err := client.New(function.Function{Root: root, Runtime: "invalid"}); err == nil {
...
func TestNamed(t *testing.T) {
	...
	if err := client.New(function.Function{Root: root, Name: name}); err != nil {
		t.Fatal(err)
	}
...
func TestDeriveImageDefaultRegistry(t *testing.T) {
	...
	client := function.New(function.WithRegistry("alice"))
	if err := client.New(function.Function{Root: root}); err != nil {
		t.Fatal(err)
	}
	...
	expected := function.DefaultRegistry + "/alice/" + f.Name + ":latest"
	if f.Image != expected {
		t.Fatalf("expected image '%v' got '%v'", expected, f.Image)
	}
}
...
func TestNewDelegates(t *testing.T) {
	...
	client := function.New(
		function.WithRegistry(TestRegistry),
		function.WithBuilder(builder),   // builds an image
		function.WithPusher(pusher),     // pushes images to a registry
		function.WithDeployer(deployer), // deploys images as a running service
	)
	...
	builder.BuildFn = func(f function.Function) error {
	...
	pusher.PushFn = func(f function.Function) (string, error) {
	...
	deployer.DeployFn = func(f function.Function) error {
...
func TestRun(t *testing.T) {
	...
	client := function.New(function.WithRegistry(TestRegistry), function.WithRunner(runner))
	if err := client.New(function.Function{Root: root}); err != nil {
		t.Fatal(err)
	}
...
func TestUpdate(t *testing.T) {
	...
	client := function.New(
		function.WithRegistry(TestRegistry),
		function.WithBuilder(builder),
		function.WithPusher(pusher),
		function.WithDeployer(deployer))
...
func TestRemoveByPath(t *testing.T) {
	...
	if err := client.Remove(function.Function{Root: root}); err != nil {
		t.Fatal(err)
	}
...
func TestRemoveByName(t *testing.T) {
	...
	if err := client.Remove(function.Function{Name: expectedName}); err != nil {
		t.Fatal(err)
	}

	// Run remove with a name and a root, which should be ignored in favor of the name.
	if err := client.Remove(function.Function{Name: expectedName, Root: root}); err != nil {
		t.Fatal(err)
	}
...
func TestList(t *testing.T) {
	...
	client := function.New(function.WithLister(lister)) // lists deployed Functions.
...
func TestListOutsideRoot(t *testing.T) {
	...
	client := function.New(function.WithLister(lister))
