...
locally with `func run`, and running on the cluster with `func deploy`. Reading is
...
also tested.  Language-specific scenarios beyond those permutations are named
`TestMatrix_<Runtime>_*` (for example `TestMatrix_Python_*`) so they run on
the corresponding runtime matrix job rather than in Core, and can be selected
