...
Remote, Podman, Matrix, and Config CI.

Core tests cover the go-centric CRUDL path: Create, Read, Update, Delete and
List.  Creation is implemented as `func init`, running the function locally
with `func run`, and running on the cluster with `func deploy`. Reading is
...
templates (repositories) and other core variants.  Language-specific depth
(Python signatures, userdeps, legacy parliament, etc.) lives under Matrix,
not Core.
...
Matrix tests cover operations that differ by language runtime.  The primary
permutations (each runtime × template × builder) check create, local run,
deploy, and remote deploy.  In addition, language-specific scenarios are
named `TestMatrix_<Runtime>_*` (for example `TestMatrix_Python_*`) so they
run on the corresponding runtime matrix job rather than in Core, and can be
selected with a prefix filter.

Config CI tests verify that GitHub Actions workflows generated by
`func config ci` deploy a function successfully. They use nektos/act
to execute the generated workflow YAML against a real cluster, iterating
over supported runtimes. Requires `act` (installed via `hack/binaries.sh`).
Since `act` runs with `-self-hosted`, the host machine must have the
runtime toolchains installed:

- **Go**: `go` on PATH
- **Node / TypeScript**: `node` and `npm` on PATH
- **Python**: `python` and `pip` on PATH
- **Quarkus**: Full JDK 17+ (not just JRE) with `javac` on PATH
...
`FUNC_E2E_CONFIG_CI`: enables Config CI tests that deploy functions via
generated GitHub Actions workflows using `act`. Requires `act` to be
installed (via `hack/binaries.sh`). Defaults to "false".
...
- TestRemote
- TestMatrix
- TestMatrix_Python  (Python-specific matrix scenarios; requires FUNC_E2E_MATRIX=true)
- TestConfigCI
