...
// at root. keda defaults to the http scaler when no scale.keda is given, so
// this is not required to reach a deployable state; it makes the http trigger
// explicit in func.yaml for tests whose assertions read it back.
