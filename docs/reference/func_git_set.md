...
```
  -b, --builder string             Builder to use when creating the function's container. Currently supported builders are "pack" and "s2i". (default "pack")
      ...
      --config-cluster             Configure cluster resources (credentials and config on cluster). (default true)
      --config-local               Configure local resources (pipeline templates). (default true)
      --config-remote              Configure remote resources (webhook on the Git provider side). (default true)
