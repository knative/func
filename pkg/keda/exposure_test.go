...
func Test_interceptorNamespace(t *testing.T) {
	interceptorIn := func(ns string) *corev1.Service {
	...
	tests := []struct {
		...
		installed []string
		want      string
		// wantPresence pins the value the deploy path reads to decide whether
		// to refuse. Nothing pinned it before: every call site in this file
		// discarded it, so the flag could have been inverted and stayed green.
		wantPresence interceptorPresence
	}{
		{
			name: "CMA on OpenShift", openShift: true,
			installed: []string{interceptorNamespaceOpenShift},
			want:      interceptorNamespaceOpenShift, wantPresence: interceptorConfirmed,
		},
		{
			// The case platform detection alone gets wrong, and the reason
			// for looking rather than inferring.
			name: "upstream keda on OpenShift", openShift: true,
			installed: []string{interceptorNamespaceUpstream},
			want:      interceptorNamespaceUpstream, wantPresence: interceptorConfirmed,
		},
		{
		...
		{
			name: "CMA namespace present off OpenShift", openShift: false,
			installed: []string{interceptorNamespaceOpenShift},
			want:      interceptorNamespaceOpenShift, wantPresence: interceptorConfirmed,
		},
		{
			// Both installed: the platform breaks the tie.
		...
		{
			// Both candidates answered NotFound. Definite, and the branch that
			// must not share a message with the denied one.
			name: "neither installed: platform default", openShift: true,
			installed: nil,
			want:      interceptorNamespaceOpenShift, wantPresence: interceptorAbsent,
		},
		{
			name: "neither installed off OpenShift: platform default", openShift: false,
			installed: nil,
			want:      interceptorNamespaceUpstream, wantPresence: interceptorAbsent,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, ns := range tt.installed {
				objects = append(objects, interceptorIn(ns))
			}

			got, presence := interceptorNamespace(t.Context(), fake.NewClientset(objects...))
			if got != tt.want {
				t.Errorf("interceptorNamespace() = %q, want %q", got, tt.want)
			}
			if presence != tt.wantPresence {
				t.Errorf("interceptorNamespace() presence = %v, want %v", presence, tt.wantPresence)
			}
		})
	}
}

// Test_interceptorNamespace_AllDeniedIsNoRegression: a restricted account
// that cannot read either candidate resolves exactly as func did before any
// probe existed, the platform default. That is the property that made the
// probe safe to add, not the one that makes it useful.
//
...
func Test_interceptorNamespace_AllDeniedIsNoRegression(t *testing.T) {
	...
	got, presence := interceptorNamespace(t.Context(), clientset)
	...
	// The namespace is the same as the absent case; the presence is what must
	// differ, since it is the only thing telling the caller it never looked.
	if presence != interceptorUndetermined {
		t.Errorf("presence = %v, want interceptorUndetermined: a denied lookup must not report absence", presence)
	}
}
...
func Test_interceptorNamespace_DeniedBeatsRuledOut(t *testing.T) {
	...
	if presence != interceptorUndetermined {
		t.Errorf("presence = %v, want interceptorUndetermined: one candidate was never ruled out", presence)
	}
}
...
func Test_interceptorExposure(t *testing.T) {
	...
	d := NewDeployer()
	e := d.interceptorExposure(fn.Function{Name: "f", Runtime: "go"}, "ns", interceptorNamespaceOpenShift)

	if e.TargetService != interceptorServiceName {
		t.Errorf("TargetService = %q, want the interceptor Service %q", e.TargetService, interceptorServiceName)
	}
	if e.TargetService == "f" {
		t.Error("exposure targets the function's own Service, which bypasses the interceptor")
	}
...
// the suffix pushes it past a DNS-1035 label's 63 characters. Checking it up
// front turns an opaque API rejection, arriving after the Deployment and
// Service already exist, into a refusal before anything is created.
func Test_validateBridgeName(t *testing.T) {
	...
	t.Run("the longest name func itself accepts", func(t *testing.T) {
		...
		if err := d.validateBridgeName(fn.Function{Name: name(63)}); err == nil {
