...
func NewServingClient(k8sClient *k8s.Client, namespace string) (clientservingv1.KnServingClient, error) {
	restConfig, err := k8sClient.ClientConfig()
	if err != nil {
...
func NewEventingClient(k8sClient *k8s.Client, namespace string) (clienteventingv1.KnEventingClient, error) {
	restConfig, err := k8sClient.ClientConfig()
	if err != nil {
		return nil, fmt.Errorf("failed to create new serving client: %v", err)
	}
