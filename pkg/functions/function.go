...
// WarnIfLegacyS2IScaffolding writes a warning to w when the func-generated
// legacy .s2i/bin/assemble file is present at the function root.
// Prior to https://github.com/knative/func/pull/3436, func wrote S2I
// scaffolding for go/python to <func-root>/.s2i/bin/assemble; it now lives
// at .func/build/bin/assemble.  The stale file can interfere with other
// builders (e.g. pack erroneously detects the assemble script).
// The check is intentionally scoped to scaffolded runtimes (go/python) and
// to the exact file func generated, so user-managed .s2i directories are not
// flagged.
func WarnIfLegacyS2IScaffolding(f Function, w io.Writer) {
	...
	if _, err := os.Stat(legacyAssemble); err == nil {
		fmt.Fprintf(w, "Warning: a '.s2i/bin/assemble' file was detected at the function root. "+
			"This appears to be leftover func-generated scaffolding from before it was moved to '%s/%s/bin/assemble'. "+
			"It may interfere with other builders. "+
			"If it was generated by func, consider removing '.s2i/' after verifying it is no longer needed.\n",
			RunDataDir, BuildDir)
	}
}
