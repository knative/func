...
cross_build() {
  ...
  echo "   🐧 kn-plugin-func-linux-amd64"
  GOOS=linux GOARCH=amd64 go build -mod=vendor -ldflags "${ld_flags}" -o ./kn-plugin-func-linux-amd64 ./cmd/func || failed=1
  echo "   💪 kn-plugin-func-linux-arm64"
  GOOS=linux GOARCH=arm64 go build -mod=vendor -ldflags "${ld_flags}" -o ./kn-plugin-func-linux-arm64 ./cmd/func || failed=1
  echo "   🍏 kn-plugin-func-darwin-amd64"
  GOOS=darwin GOARCH=amd64 go build -mod=vendor -ldflags "${ld_flags}" -o ./kn-plugin-func-darwin-amd64 ./cmd/func || failed=1
  echo "   🎠 kn-plugin-func-windows-amd64.exe"
  GOOS=windows GOARCH=amd64 go build -mod=vendor -ldflags "${ld_flags}" -o ./kn-plugin-func-windows-amd64.exe ./cmd/func || failed=1
  echo "   Z  kn-plugin-func-linux-s390x"
  GOOS=linux GOARCH=s390x go build -mod=vendor -ldflags "${ld_flags}" -o ./kn-plugin-func-linux-s390x ./cmd/func || failed=1
  echo "   P  kn-plugin-func-linux-ppc64le"
  GOOS=linux GOARCH=ppc64le go build -mod=vendor -ldflags "${ld_flags}" -o ./kn-plugin-func-linux-ppc64le ./cmd/func || failed=1
