    ...
    if [[ -x $(command -v "nix") || -x $(command -v "nixos-rebuild") ]]; then
        ...
        if command -v docker &> /dev/null; then
          if [[ "$(uname)" == "Darwin" ]]; then
            ...
            echo "Please confirm \"registry.localtest.me\" is specified as an insecure registry in the docker config file."
          else
            ...
            echo "    enable = true;"
            echo "    daemon.settings.insecure-registries = [ \"registry.localtest.me\" ];"
            echo "  };"
          fi
        fi
...
set_registry_insecure() {
    ...
    patch=".\"insecure-registries\" = [\"registry.localtest.me\"]"
...
set_registry_insecure_podman() {
    ...
    if [ -f "$USER_CONFIG_FILE" ]; then
        ...
        if mkdir -p "$USER_CONFIG_DIR" 2>/dev/null; then
            ...
            echo "location = \"registry.localtest.me\"" >> "$CONFIG_FILE"
            echo "insecure = true" >> "$CONFIG_FILE"
            echo "Successfully created Podman registry configuration for registry.localtest.me"
            return 0
        else
    ...
    # Check if registry.localtest.me is already configured
    ALREADY_CONFIGURED=false
    if [ "$NEED_SUDO" = true ]; then
        if sudo grep -q "registry.localtest.me" "$CONFIG_FILE" 2>/dev/null; then
            ALREADY_CONFIGURED=true
        fi
    else
        if grep -q "registry.localtest.me" "$CONFIG_FILE" 2>/dev/null; then
            ALREADY_CONFIGURED=true
        fi
    fi

    if [ "$ALREADY_CONFIGURED" = true ]; then
        echo "registry.localtest.me is already configured in $CONFIG_FILE"
        return 0
    fi
    ...
    if [ "$IS_V2_FORMAT" = true ]; then
        # Use v2 format
        echo "Adding registry.localtest.me as insecure registry (v2 format)"
        if [ "$NEED_SUDO" = true ]; then
        ...
        # Use v1 format
        echo "Adding registry.localtest.me as insecure registry (v1 format)"
        if [ "$NEED_SUDO" = true ]; then
            ...
            if sudo grep -q "^\[registries.insecure\]" "$CONFIG_FILE" 2>/dev/null; then
                ...
                sudo sed -i '/^\[registries.insecure\]/a registries = ["registry.localtest.me"]' "$CONFIG_FILE"
            else
                # Section doesn't exist, add it
                echo -e "\n[registries.insecure]\nregistries = [\"registry.localtest.me\"]" | sudo tee -a "$CONFIG_FILE" > /dev/null
            fi
        else
            # Check if [registries.insecure] section exists
            if grep -q "^\[registries.insecure\]" "$CONFIG_FILE" 2>/dev/null; then
                # Section exists, add to the registries list
                sed -i '/^\[registries.insecure\]/a registries = ["registry.localtest.me"]' "$CONFIG_FILE"
            else
                # Section doesn't exist, add it
                echo -e "\n[registries.insecure]\nregistries = [\"registry.localtest.me\"]" >> "$CONFIG_FILE"
            fi
        fi
    fi
